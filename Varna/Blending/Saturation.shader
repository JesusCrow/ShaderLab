...
// Trying to fake Saturation blending of Photoshop without HCL
// by Varna 2017

Shader "_Varna/Blending/Saturation"
{
	Properties
	{
...
half4 fragmentFunction( v2f IN ) : COLOR
{
	...
	float3 	C 			= RGBtoHCV(colorFG.xyz);
	float3 	HL 			= RGBtoHSL(colorBG.xyz);
	float3 	colorHCL 	= float3(HL.x, C.y, HL.z);
