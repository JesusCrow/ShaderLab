...
// Trying to fake Hue blending of Photoshop without HCL
// by Varna 2017

Shader "_Varna/Blending/Hue"
{
	Properties
	{
...
half4 fragmentFunction( v2f IN ) : COLOR
{
	...
	float3 	H 			= RGBtoHCV(colorFG.xyz);
	float3 	C 			= RGBtoHCV(colorBG.xyz);
	float3 	L 			= RGBtoHSL(colorBG.xyz);
	float3 	colorHCL 	= float3(H.x, C.y, L.z);
