...
Shader "_Varna/BlenWithFx"
{
	Properties
	{
		_MainColor ( "Tint Color", Color ) = ( 1.0, 1.0, 1.0, 1.0 )
		_MainTex ( "Texture", 2D ) = "white" {}
		_WidthTex		("Width Tex", float) = 1
		_HeightTex		("Height Tex", float) = 1
...
half4 fragmentFunction( v2f IN ) : COLOR
{
	...
	// Texture tiling (Does not work properly,
	// probably should be implemented inside vertex)
	IN.uv.x = IN.uv.x * _WidthTex;
	IN.uv.y = IN.uv.y * _HeightTex;

	// Getting textures that we want to blend into uniforms
	float4 	colorBG		= _MainColor;
	float4 	colorFG		= tex2D(_MainTex, IN.uv);

	// Translating RGB texture values to different color modes
	float3 	HC 			= RGBtoHCV(colorBG.xyz);
	float3 	L 			= RGBtoHSL(colorFG.xyz);

	// Adjusting HCL offsets
	L.z 				= L.z + _Luma;
	HC.xy 				= HC.xy + fixed2(_Hue, _Chroma);

	// Hack of using HSL converted to convert HCL values to RGB
	half3 	color 		= HSLtoRGB(float3(HC.x, HC.y, L.z));

	// Texture tiling reset (Does not work properly,
	// probably should be implemented inside vertex)
	IN.uv.x = IN.uv.x / _WidthTex;
	IN.uv.y = IN.uv.y / _HeightTex;
	...
	// Setting all unwanted (above y) pixels to alfa(0)
